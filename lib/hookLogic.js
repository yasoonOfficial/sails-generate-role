...
var logicFunctions = {
    ...
    attachRoles: function attachRoles(roles) {
        //Precheck roles
        _.forEach(roles, function (role, roleKey) {
            //Check that each role has "hasRole" method
            if (!role.hasRole || typeof (role.hasRole) !== 'function')
                throw 'Role ' + roleKey + ' does not implement "hasRole" method!';
        });
