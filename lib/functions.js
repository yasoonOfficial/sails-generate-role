...
var functions = {
    ...
    findRoles: function(request, cb) {
        //Check each role
        var roleKeys = _.keys(sails.roles);
        request._roles = [];
        var processNextRole = function (i) {
            sails.roles[roleKeys[i]].hasRole(request, function (err, roles) {
                if (err) {
                    //Do not has the role, continue
                    if (++i < roleKeys.length)
                        return processNextRole(i);
                }

                //For systems with a lot of roles, single role can supply all roles in one fetch
                if (roles) {
                ...
                //Just a normal role
                request._roles.push(roleKeys[i]);
                if (++i < roleKeys.length)
                    return processNextRole(i);
                else
                    cb();
    ...
    processRestrict: function processRestrict(deferred, request, model, restrictRoles, restrictValues, cb) {
        ...
        var criteria = fromDeferredApi ? deferred._criteria : request.params.all().where;
